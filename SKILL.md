...
- 输出前必须完成 `wang-references/` 深读，不得只依赖 5.1 简表快速判断。
- 完整解读时，本命/流年/流月/流日每层至少给出 3 条关键判断，且每条都附证据链。
- 若平台输出长度受限：在当前消息末尾写`【未完成，下一部分继续】`，并按同一模板编号续写，直到 1~8 节全部完成；不得压缩成摘要收尾。
...
- 禁止只搬运术语；必须把参考观点转成可理解的结构化解读与行动建议

### 7.4 输出前自检

- 是否完成了 7.1 的基础必读与必要条件补读
- 是否按模板 1~8 节完整输出，且未省略层级
- 是否所有关键结论都附有证据链
- 若消息被截断，是否显式续写直到模板完成
